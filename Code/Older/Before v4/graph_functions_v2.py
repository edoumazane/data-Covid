...
from dicts_paths_v2 import date, date_text, reg_2lignes, fname_output_synth, dep_name

def format_graph(ax, ymin, ymax, x_axis = 'without', y_labels = "to_the_left"):
    """
    formats the graph
    ax: axes object
    ymin, ymax: integer
...
def figure_line(row, nrow, axs, d, column_to_plot, main_color, ymin, ymax, title, regions):
    ...
    format_graph(ax, ymin, ymax, x_axis = ('regular' if (row == nrow - 1) else 'without'), y_labels = ('to_the_left' if (col == 0) else 'to_the_right' if (col == ncol - 1) else 'without'))
    # Légende
...
def produce_fig_lines(d, regions, regions_ordered, numero):
    ...
    figure_line(row = 0,
                ...
                ymin = -400, 
                ymax = 11900,
                title = '$\\bf{Tests}$ pratiqués par semaine\npour 100 000 personnes', 
                regions = regions)  
    ...
    figure_line(row = 1,
                ...
                ymin = -40, 
                ymax = 1190,
                title = 'Cas $\\bf{positifs}$ par semaine\npour 100 000 personnes', 
                regions = regions)  
    ...
    figure_line(row = 2,
                ...
                ymin = -15, 
                ymax = 379,
                title = 'Patients $\\bf{hospitalisés}$\npour 100 000 personnes', 
                regions = regions)
    ...
    figure_line(row = 3,
                ...
                ymin = -3, 
                ymax = 76,
                title = 'Patients en $\\bf{réanimation}$\npour 100 000 personnes', 
                regions = regions)
    ...
    figure_line(row = 4,
                ...
                ymin = -3, 
                ymax = 76,
                title = '$\\bf{Décès}$ à l\'hôpital par semaine\npour 100 000 personnes', 
                regions = regions)
...
def produce_fig(ymin, ymax, d, column_to_plot, main_color, title, regions_ordered, fname):
    ...
    format_graph(ax, ymin, ymax, x_axis = ('complete'), y_labels = 'to_the_left')
    ...
    for i, region in enumerate(regions_ordered):
        ...
        # ax.plot(dplot.jour, dplot[column_to_plot], c = "firebrick", linewidth = 1)
        format_graph(ax, ymin, ymax, x_axis = 'without', y_labels = 'without')
