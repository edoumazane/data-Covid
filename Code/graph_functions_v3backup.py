...
def format_graph(ax, **kwargs, x_axis = 'complete', y_labels = "to_the_left"):
    """
    formats the graph
    ax: axes object
    ymin, ymax: integer
...
def simple_figure(d, entity, column_to_plot, **kwargs, autoscale = False):
    if autoscale:
        ymin = d[d.entity == entity][column_to_plot].min()
        ymax = d[d.entity == entity][column_to_plot].max()
    fig, ax = plt.subplots(1, 1, figsize = (12, 3))
    plot_three_curves(ax, d, entity, column_to_plot, **kwargs[column_to_plot])
...
def produce_fig_lines(d, regions, regions_ordered, numero):
    ...
    fname = path_output + str(numero) + '.pdf'
    fig.savefig(fname, pad_inches = 0)
    fname = path_output + str(numero) + '.png'
    fig.savefig(fname, pad_inches = 0)
